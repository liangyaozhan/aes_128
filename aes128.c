...
void aes128_set_key(struct aes128 *p_this, const uint8_t _key[16])
{
    const uint8_t *key = _key;
    uint8_t *roundkeys = p_this->roundkeys;
    uint8_t temp[4];
...
{
    const uint8_t *roundkeys = p_this->roundkeys;
    uint8_t i;
...
void aes128_destory(struct aes128 *p_this)
{
    (void)p_this;
}
